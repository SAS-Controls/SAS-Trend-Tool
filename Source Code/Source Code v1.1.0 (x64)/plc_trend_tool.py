...
# Integer types that support bit-level access (tag.X syntax in Logix)
# Maps data type -> number of bits
BIT_ADDRESSABLE_TYPES = {
...
def smart_tag_name(tag, all_tags=None):
    """Return a display-friendly tag name that preserves enough context to be unique.
    Uses full tag name by default. Only shortens if the full name is very long
    and the short form is still unique among the given tag set."""
    if not tag:
        return tag
    # For tags with Program: prefix, strip it for display
    if tag.startswith("Program:"):
        dot_idx = tag.find(".")
        if dot_idx > 0:
            tag = tag[dot_idx + 1:]
    # If tag is short enough, use it as-is
    if len(tag) <= 40:
        return tag
    # For very long tags, try to shorten while staying unique
    short = tag.split(".")[-1] if "." in tag else tag
    if all_tags:
        # Check if short name is unique
        short_names = [t.split(".")[-1] if "." in t else t for t in all_tags]
        if short_names.count(short) > 1:
            # Not unique — use last two segments
            parts = tag.split(".")
            if len(parts) >= 2:
                return ".".join(parts[-2:])
            return tag
    return short if len(tag) > 40 else tag
...
class PLCTrendTool(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Display settings (persisted via settings file)
        self._show_legend = self.settings.get("show_legend", True)
        self._show_x_scale = self.settings.get("show_x_scale", True)
        self._show_x_grid = self.settings.get("show_x_grid", True)
        self._allow_scrolling = self.settings.get("allow_scrolling", True)
    ...
    def _build_sidebar(self):
        ...
        self._storage_info_label = ctk.CTkLabel(bottom, text="",
                                                 font=(FONT_FAMILY, FONT_SIZE_TINY),
                                                 text_color=TEXT_MUTED, anchor="w")
        self._storage_info_label.pack(fill="x", padx=4, pady=(0, 4))
    ...
    def _show_line_properties(self, tags_to_edit):
        ...
        for tag in tags_to_edit:
            ...
            display_name = smart_tag_name(tag, tags_to_edit)
            ctk.CTkLabel(hdr, text=display_name,
    ...
    def _show_chart_properties(self):
        ...
        x_scale_var = ctk.BooleanVar(value=self._show_x_scale)
        ...
        x_grid_var = ctk.BooleanVar(value=self._show_x_grid)
        ...
        if tags:
            ...
            for i, tag in enumerate(tags):
                ...
                display_name = smart_tag_name(tag, tags)
                ctk.CTkLabel(row1, text=display_name, font=(FONT_FAMILY, FONT_SIZE_SMALL, "bold"),
                             text_color=TEXT_PRIMARY).pack(side="left")
        ...
        legend_var = ctk.BooleanVar(value=self._show_legend)
        ...
        scroll_var = ctk.BooleanVar(value=self._allow_scrolling)
        ...
        def apply_and_close():
            ...
            self._show_legend = legend_var.get()
            self._show_x_scale = x_scale_var.get()
            self._show_x_grid = x_grid_var.get()
            self._allow_scrolling = scroll_var.get()
            ...
            self.settings["smart_cursor"] = self._cursor_enabled
            self.settings["show_legend"] = self._show_legend
            self.settings["show_x_scale"] = self._show_x_scale
            self.settings["show_x_grid"] = self._show_x_grid
            self.settings["allow_scrolling"] = self._allow_scrolling
            save_settings(self.settings)

            # Always rebuild to apply all display settings (legend, grid, scale, etc.)
            self._rebuild_chart()
            dlg.destroy()
    ...
    def _on_chart_mouse_move(self, event):
        ...
        for i, (tag, (t_arr, v_arr)) in enumerate(chart_data.items()):
            if idx < len(v_arr):
                ...
                color = lp["color"]
                display_name = smart_tag_name(tag, tags_list)
                if val is not None and val == val:  # val==val is False for NaN
                    ...
                    text_lines.append(f"\u25CF {display_name}: {val_str}")
                else:
    ...
    def _insert_tag_item(self, parent, tag, full_path, display_name=None):
        ...
        if is_slc_file:
            ...
            is_trendable = tag.get("trendable", False)
            bit_count = BIT_ADDRESSABLE_TYPES.get(tag["dataType"], 0) if is_trendable else 0
            if bit_count > 0:
                # Integer type — make expandable for bit-level access
                item_id = self.tag_tree.insert(parent, "end", text="\u25B6 " + name,
                                               values=(tag["dataType"],), tags=("struct",))
                self._struct_items[item_id] = {
                    "path": full_path,
                    "dtv": 0,
                    "array": 0,
                    "populated": False,
                    "_bit_addressable": True,
                ...
                tt = "trendable" if is_trendable else "disabled"
    ...
    def _on_tree_expand(self, event):
        ...
        # === Logix bit-addressable element (INT, DINT, etc.) ===
        elif info.get("_bit_addressable"):
            self._populate_bit_addressable(item, info)
        # === Logix UDT/struct expansion ===
        elif info.get("atomic_array"):
            # Array of atomic type — show indexed elements
            dt = info.get("dataType", "UNKNOWN")
            bit_count = BIT_ADDRESSABLE_TYPES.get(dt, 0)
            max_show = min(info["array"], 100)
            struct_fg = resolve_color(("#5080B0", "#7AB0D8"))
            for i in range(max_show):
                el_path = f"{info['path']}[{i}]"
                if bit_count > 0:
                    # Integer type — make expandable so user can drill to bit level
                    arr_item = self.tag_tree.insert(item, "end", text=f"\u25B6 [{i}]",
                                                     values=(dt,), tags=("struct",))
    ...
    def _populate_bit_addressable(self, parent_item, info):
        """Populate bit-level children for a Logix integer-type tag (INT, DINT, etc.).
        Shows the whole word as trendable, plus individual bits (.0 through .N)."""
        addr = info["path"]
        dt = info.get("dataType", "DINT")
        bit_count = info.get("_bit_count", 32)
        primary = resolve_color(TEXT_PRIMARY)

        # First: trendable whole word
        prefix = "\u2611 " if addr in self.selected_tags else "\u2610 "
        display = addr.split(".")[-1] if "." in addr else addr
        # Show array index if present
        if "[" in addr:
            parts = addr.rsplit("[", 1)
            display = f"[{parts[1]}  (whole word)" if len(parts) == 2 else f"{display}  (whole word)"
        else:
            display = f"{display}  (whole word)"
        self.tag_tree.insert(parent_item, "end", text=f"{prefix}{display}",
                             values=(dt,), tags=("trendable", addr))
        self.tag_data_types[addr] = dt

        # Then: individual bits
        for bit in range(bit_count):
            bit_addr = f"{addr}.{bit}"
            prefix = "\u2611 " if bit_addr in self.selected_tags else "\u2610 "
            self.tag_tree.insert(parent_item, "end", text=f"{prefix}.{bit}",
                                 values=("BOOL",), tags=("trendable", bit_addr))
            self.tag_data_types[bit_addr] = "BOOL"
        self.tag_tree.tag_configure("trendable", foreground=primary)
        ...
        for field in udt_def["fields"]:
            ...
            if is_struct:
                ...
                if array_count and is_trendable:
                    ...
                    bit_count = BIT_ADDRESSABLE_TYPES.get(field["dataType"], 0)
                    if bit_count > 0:
                        # Integer type — make expandable for bit-level access
                        child = self.tag_tree.insert(parent_item, "end",
                                                      text=f"\u25B6 {field['name']}",
                        ...
                        # BOOL, REAL, LREAL — directly trendable
    ...
    def _estimate_data_size(self):
        """Estimate the in-memory data size and approximate export file size.
        Returns (in_memory_bytes, export_estimate_bytes, display_string)."""
        with self.trend._lock:
            point_count = len(self.trend.data)
            tag_count = len(self.trend.tags)
        if point_count == 0 or tag_count == 0:
            return 0, 0, ""
        # Estimate: each data point stores a timestamp string (~24 chars) +
        # a dict of tag values (tag name ~30 chars avg + value ~8 bytes each)
        # JSON overhead for pytrend export adds ~40% formatting
        avg_tag_name_len = sum(len(t) for t in self.trend.tags) / max(tag_count, 1)
        bytes_per_point = 30 + tag_count * (avg_tag_name_len + 12)  # timestamp + values
        in_memory = int(point_count * bytes_per_point)
        # pytrend export is JSON with indentation — roughly 1.4x in-memory estimate
        export_est = int(in_memory * 1.4)
        # Format nicely
        if export_est < 1024:
            size_str = f"{export_est} B"
        elif export_est < 1024 * 1024:
            size_str = f"{export_est / 1024:.1f} KB"
        elif export_est < 1024 * 1024 * 1024:
            size_str = f"{export_est / (1024 * 1024):.1f} MB"
        else:
            size_str = f"{export_est / (1024 * 1024 * 1024):.2f} GB"
        return in_memory, export_est, size_str

    def _update_storage_info(self):
        """Update the sidebar storage estimate label."""
        _, export_est, size_str = self._estimate_data_size()
        if not size_str:
            self._storage_info_label.configure(text="")
            return
        # Color-code based on size: green < 50MB, yellow < 200MB, red >= 200MB
        if export_est < 50 * 1024 * 1024:
            color = TEXT_MUTED
        elif export_est < 200 * 1024 * 1024:
            color = STATUS_WARN
        else:
            color = STATUS_ERROR
        self._storage_info_label.configure(
            text=f"\U0001F4BE Est. export: ~{size_str}",
            text_color=color)
